    ...
    for (const type of ['preferred', 'active', 'transient', 'reserve']) {
        ...
        for (const { stakeAddress, voteAddress, lamports } of filteredAccounts) {
            ...
            if (!skipFee && !inverseFee.numerator.isZero()) {
                availableForWithdrawal = divideBnToNumber(new BN__default["default"](availableForWithdrawal).mul(inverseFee.denominator), inverseFee.numerator);
            }
...
function calcPoolTokensForDeposit(stakePool, stakeLamports) {
    ...
    return Math.floor(divideBnToNumber(new BN__default["default"](stakeLamports).mul(stakePool.poolTokenSupply), stakePool.totalLamports));
}
/**
...
async function stakePoolInfo(connection, stakePoolAddress) {
    ...
    const withdrawAuthority = await findWithdrawAuthorityProgramAddress(STAKE_POOL_PROGRAM_ID, stakePoolAddress);
    const minimumReserveStakeBalance = (await connection.getMinimumBalanceForRentExemption(web3_js.StakeProgram.space)) + 1;
